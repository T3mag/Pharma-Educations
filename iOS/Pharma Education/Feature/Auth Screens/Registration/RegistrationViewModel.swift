...
final class RegistrationViewModel: RegistrationViewModelProtocol {
    ...
    func register(email: String,
        ...
        guard passwordMatch(password, confirmPassword) else {
            onError?(Texts.Errors.passwordNoMatched)
            return
        }
        
        let registartionUserInfo = UserInfoFirestoreItem(
            email: email,
            fullName: fullName
        )
                                                        
        authService.register(
            registrationUserInfo: registartionUserInfo,
            password: password
        ) { result in
            switch result {
            case .success:
    ...
    private func checkValidEmail(_ email: String) -> Bool {
        ...
        let pattern = RegistrationTexts.emailPattern
