...
final class SimulatorTableViewHandler: NSObject, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .title:
            guard let titleCell = tableView.dequeueReusableCell(withIdentifier: SimulatorsTableViewTitleCell.reuseIdentifire, for: indexPath) as? SimulatorsTableViewTitleCell else {
                return UITableViewCell()
            }
            return titleCell
        case.item(let item):
            guard let simulatorCell = tableView.dequeueReusableCell(
                withIdentifier: SimulatorsTableViewGameCell.reuseIdentifire,
